...
close.addEventListener('click', () => {
  modal_container.classList.remove('show');
    ...
    console.log(`¡Espero que te sientas mal por hacer trampa!\nDe todos modos, tu palabra es "${Word}"`);
    ...
    const guessWord = async () => {
        ...
        if (guessedWord === Word) {
            ...
            sendMessage("success", "Felicistaciones! Adivinaste la palabra!");
            await delay(5000);
            resetGame();
        }
    ...
    document.addEventListener("keydown", (event) => {
        if (event.keyCode > 165 || (event.keyCode > 64 && event.keyCode < 91)) {
            if (index < maxRows) {
                const row = document.getElementById(`${currentRow}`);
                row.getElementsByClassName(
                    `${index}`
